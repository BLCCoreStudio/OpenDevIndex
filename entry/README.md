# Actix Web

> Powerful Rust web framework built on the Actix ecosystem, providing asynchronous HTTP servers, routing, middleware, extractors, and WebSocket support.

## What it is

Actix Web is indexed as a **framework**. Its stable OpenDevIndex address is `framework/actix-web`; taxonomy facets are kept separate from that address so classification can improve without breaking links.

## Why it matters

The module focuses on the technology's practical role, high-signal characteristics, common use cases, and authoritative references. Fast-changing details should be verified against the sources below rather than inferred from stale copies.

## Typical use cases

- Build asynchronous Rust web services
- Create typed request handlers and middleware
- Serve HTTP and WebSocket workloads with a native Rust stack

## Key points

- Actix Web uses Rust asynchronous programming
- Extractors convert request state into typed handler inputs
- The framework supports middleware and streaming response patterns

## Taxonomy

- Kind: `framework`
- Domains: `systems`, `web`
- Deployment: `self-hosted`, `service`
- License metadata: `not yet curated`

## Primary links

- Homepage: https://actix.rs/
- Repository: https://github.com/actix/actix-web

## Verified sources

- [Actix official site](https://actix.rs/) — `official`
- [Actix Web source repository](https://github.com/actix/actix-web) — `repository`

## Verification

The catalog metadata and source references for this module were reviewed on **2026-08-31**. Automated checks validate structure and source reachability; factual updates still require source-backed editorial review.

## Maintenance

Update this independently versioned module when material facts, project status, canonical documentation, or important trade-offs change. Preserve the stable module address unless a compatibility migration is explicitly documented.
