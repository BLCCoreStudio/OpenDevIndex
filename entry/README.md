# WebGPU

> Web standard and API family for modern GPU access from web applications, providing programmable rendering and general-purpose compute capabilities.

## What it is

WebGPU is indexed as a **standard**. Its stable OpenDevIndex address is `standard/webgpu`; taxonomy facets are kept separate from that address so classification can improve without breaking links.

## Why it matters

The module focuses on the technology's practical role, high-signal characteristics, common use cases, and authoritative references. Fast-changing details should be verified against the sources below rather than inferred from stale copies.

## Typical use cases

- Render advanced graphics in web applications
- Run GPU compute workloads from web environments
- Build portable browser-based visualization engines

## Key points

- Designed around modern explicit GPU APIs
- Includes a shading language called WGSL
- Browser implementations mediate access for safety and portability

## Taxonomy

- Kind: `standard`
- Domains: `graphics`, `web`
- Deployment: not yet curated
- License metadata: `not yet curated`

## Primary links

- Homepage: https://www.w3.org/TR/webgpu/
- Repository: https://github.com/gpuweb/gpuweb

## Verified sources

- [W3C WebGPU specification](https://www.w3.org/TR/webgpu/) — `standard`
- [GPU for the Web repository](https://github.com/gpuweb/gpuweb) — `repository`

## Verification

The catalog metadata and source references for this module were reviewed on **2026-08-31**. Automated checks validate structure and source reachability; factual updates still require source-backed editorial review.

## Maintenance

Update this independently versioned module when material facts, project status, canonical documentation, or important trade-offs change. Preserve the stable module address unless a compatibility migration is explicitly documented.
