...
| [Envoy](https://github.com/BLCCoreStudio/OpenDevIndex/tree/cloud/envoy/entry) (`cloud/envoy`) | `cloud`, `networking` | High-performance open-source edge and service proxy designed for cloud-native networking, traffic management, observability, and service-mesh architectures. |
| [esbuild](https://github.com/BLCCoreStudio/OpenDevIndex/tree/tool/esbuild/entry) (`tool/esbuild`) | `build`, `developer-tools`, `web` | Extremely fast JavaScript and TypeScript bundler and minifier written in Go, commonly used as a low-level engine inside modern development tooling. |
| [ESLint](https://github.com/BLCCoreStudio/OpenDevIndex/tree/tool/eslint/entry) (`tool/eslint`) | `developer-tools`, `web` | Pluggable static-analysis and linting tool for JavaScript and related ecosystems, used to detect problematic patterns and enforce project coding rules. |
| [eza](https://github.com/BLCCoreStudio/OpenDevIndex/tree/tool/eza/entry) (`tool/eza`) | `cli`, `developer-experience` | Modern command-line directory listing utility derived from exa, adding readable defaults, colors, Git metadata, tree views, and structured file information. |
...
| [Gradle](https://github.com/BLCCoreStudio/OpenDevIndex/tree/tool/gradle/entry) (`tool/gradle`) | `build`, `developer-tools`, `software-development` | General-purpose build automation system widely used for JVM and Android projects, with programmable build logic, incremental execution, dependency management, and caching. |
...
| [OpenTofu](https://github.com/BLCCoreStudio/OpenDevIndex/tree/tool/opentofu/entry) (`tool/opentofu`) | `cloud`, `developer-tools`, `devops` | Open-source infrastructure-as-code tool created as a community-driven Terraform-compatible implementation for declaratively managing infrastructure through providers. |
| [OWASP ZAP](https://github.com/BLCCoreStudio/OpenDevIndex/tree/security/owasp-zap/entry) (`security/owasp-zap`) | `security`, `testing`, `web` | Open-source web application security scanner and intercepting proxy maintained under OWASP, used for manual testing and automated dynamic application security testing. |
| [Packer](https://github.com/BLCCoreStudio/OpenDevIndex/tree/tool/packer/entry) (`tool/packer`) | `build`, `cloud`, `devops` | HashiCorp image-building tool that creates repeatable machine images for multiple platforms from declarative templates and pluggable builders and provisioners. |
| [pipx](https://github.com/BLCCoreStudio/OpenDevIndex/tree/tool/pipx/entry) (`tool/pipx`) | `cli`, `developer-tools`, `python` | Python application installer that places each command-line package in its own isolated environment while exposing its executables on the user PATH. |
| [pnpm](https://github.com/BLCCoreStudio/OpenDevIndex/tree/tool/pnpm/entry) (`tool/pnpm`) | `developer-experience`, `developer-tools`, `web` | JavaScript package manager that uses a content-addressable store and linked dependency layout to reduce duplicated files while supporting workspaces and modern Node.js projects. |
| [Podman](https://github.com/BLCCoreStudio/OpenDevIndex/tree/tool/podman/entry) (`tool/podman`) | `containers`, `devops` | Daemonless container engine for building, running, and managing OCI containers and pods with strong support for rootless operation. |
| [Poetry](https://github.com/BLCCoreStudio/OpenDevIndex/tree/tool/poetry/entry) (`tool/poetry`) | `developer-tools`, `python` | Python packaging and dependency-management tool that manages project metadata, dependency resolution, virtual environments, builds, and package publishing workflows. |
| [Prettier](https://github.com/BLCCoreStudio/OpenDevIndex/tree/tool/prettier/entry) (`tool/prettier`) | `developer-tools`, `web` | Opinionated code formatter that parses source files and reprints them consistently, reducing style debates across JavaScript and many other supported languages. |
| [Pulumi](https://github.com/BLCCoreStudio/OpenDevIndex/tree/cloud/pulumi/entry) (`cloud/pulumi`) | `cloud`, `devops` | Infrastructure-as-code platform that lets developers define cloud resources using general-purpose programming languages and provider packages. |
| [QEMU](https://github.com/BLCCoreStudio/OpenDevIndex/tree/tool/qemu/entry) (`tool/qemu`) | `systems`, `virtualization` | Open-source machine emulator and virtualizer that can emulate full systems or individual processes across many CPU architectures. |
| [ripgrep](https://github.com/BLCCoreStudio/OpenDevIndex/tree/tool/ripgrep/entry) (`tool/ripgrep`) | `cli`, `developer-tools` | Fast recursive text-search command-line tool that respects common ignore files and is optimized for searching source trees and large directories. |
...
| [tmux](https://github.com/BLCCoreStudio/OpenDevIndex/tree/tool/tmux/entry) (`tool/tmux`) | `developer-tools` | Terminal multiplexer that creates persistent shell sessions and supports multiple windows and panes inside a single terminal connection. |
| [Traefik Proxy](https://github.com/BLCCoreStudio/OpenDevIndex/tree/tool/traefik/entry) (`tool/traefik`) | `cloud`, `containers`, `devops`, `networking` | Cloud-native reverse proxy and application proxy that discovers services dynamically and configures routing for containers and orchestration platforms. |
| [Trivy](https://github.com/BLCCoreStudio/OpenDevIndex/tree/security/trivy/entry) (`security/trivy`) | `containers`, `security` | Open-source security scanner for container images, filesystems, repositories, infrastructure configuration, dependencies, and software supply chains. |
| [uv](https://github.com/BLCCoreStudio/OpenDevIndex/tree/tool/uv/entry) (`tool/uv`) | `cli`, `developer-tools`, `python` | Fast Python package and project manager from Astral that combines environment creation, dependency installation, lockfiles, tool execution, and Python version management. |
| [Visual Studio Code](https://github.com/BLCCoreStudio/OpenDevIndex/tree/tool/vscode/entry) (`tool/vscode`) | `developer-tools` | Open-source-based code editor from Microsoft with extension support, integrated debugging, language tooling, terminals, and remote development features. |
| [Vite](https://github.com/BLCCoreStudio/OpenDevIndex/tree/tool/vite/entry) (`tool/vite`) | `build`, `developer-tools`, `web` | Frontend build tool and development server that provides fast module serving during development and optimized production builds for modern web projects. |
| [webpack](https://github.com/BLCCoreStudio/OpenDevIndex/tree/tool/webpack/entry) (`tool/webpack`) | `build`, `developer-tools`, `web` | Highly configurable JavaScript module bundler that builds dependency graphs and transforms application assets through loaders and plugins. |
| [Wireshark](https://github.com/BLCCoreStudio/OpenDevIndex/tree/tool/wireshark/entry) (`tool/wireshark`) | `developer-tools`, `networking`, `security` | Open-source network protocol analyzer with graphical and command-line tooling for capturing, decoding, filtering, and inspecting packets across a large range of protocols. |
| [YARA](https://github.com/BLCCoreStudio/OpenDevIndex/tree/security/yara/entry) (`security/yara`) | `security` | Pattern-matching tool and rule language for identifying and classifying files, memory, and malware samples using textual, binary, and structural signatures. |
...
| [Elixir](https://github.com/BLCCoreStudio/OpenDevIndex/tree/language/elixir/entry) (`language/elixir`) | `distributed-systems`, `programming-languages`, `web` | Functional programming language that runs on the Erlang VM and targets scalable, fault-tolerant, concurrent applications with modern tooling and metaprogramming. |
| [Erlang](https://github.com/BLCCoreStudio/OpenDevIndex/tree/language/erlang/entry) (`language/erlang`) | `distributed-systems`, `programming-languages` | Functional concurrent programming language and runtime ecosystem designed for fault-tolerant, distributed, soft-real-time systems that can run continuously. |
| [F#](https://github.com/BLCCoreStudio/OpenDevIndex/tree/language/fsharp/entry) (`language/fsharp`) | `programming-languages`, `software-development` | Functional-first .NET programming language that combines concise immutable-by-default code with strong typing and seamless interoperability across the .NET ecosystem. |
| [Go](https://github.com/BLCCoreStudio/OpenDevIndex/tree/language/go/entry) (`language/go`) | `cloud`, `programming-languages` | Compiled programming language designed for simple tooling, efficient concurrency, and maintainable network and systems software. |
| [Haskell](https://github.com/BLCCoreStudio/OpenDevIndex/tree/language/haskell/entry) (`language/haskell`) | `programming-languages`, `software-development` | Statically typed purely functional programming language with lazy evaluation, type inference, algebraic data types, and a strong research and library ecosystem. |
| [Java](https://github.com/BLCCoreStudio/OpenDevIndex/tree/language/java/entry) (`language/java`) | `programming-languages` | Class-based general-purpose programming language and platform centered on the Java Virtual Machine, with a large server and enterprise ecosystem. |
...
| [Perl](https://github.com/BLCCoreStudio/OpenDevIndex/tree/language/perl/entry) (`language/perl`) | `programming-languages`, `systems` | Mature dynamic programming language known for text processing, systems administration, automation, and a long-running package ecosystem centered on CPAN. |
| [PHP](https://github.com/BLCCoreStudio/OpenDevIndex/tree/language/php/entry) (`language/php`) | `programming-languages`, `web` | Widely deployed server-side programming language with a large web ecosystem, mature package tooling, and direct integration with common web servers. |
| [Python](https://github.com/BLCCoreStudio/OpenDevIndex/tree/language/python/entry) (`language/python`) | `data`, `programming-languages` | High-level general-purpose programming language known for readable syntax and a broad ecosystem spanning automation, web development, science, and AI. |
| [R](https://github.com/BLCCoreStudio/OpenDevIndex/tree/language/r/entry) (`language/r`) | `analytics`, `data`, `programming-languages` | Programming language and environment focused on statistical computing, data analysis, scientific visualization, and reproducible research workflows. |
| [Ruby](https://github.com/BLCCoreStudio/OpenDevIndex/tree/language/ruby/entry) (`language/ruby`) | `programming-languages`, `web` | Dynamic general-purpose programming language emphasizing developer productivity and expressive object-oriented code, best known for the Ruby on Rails ecosystem. |
| [Rust](https://github.com/BLCCoreStudio/OpenDevIndex/tree/language/rust/entry) (`language/rust`) | `programming-languages`, `systems` | Systems programming language focused on memory safety, performance, and concurrency without requiring a garbage collector. |
...
| Module | Domains | Summary |
| --- | --- | --- |
| [Actix Web](https://github.com/BLCCoreStudio/OpenDevIndex/tree/framework/actix-web/entry) (`framework/actix-web`) | `systems`, `web` | Powerful Rust web framework built on the Actix ecosystem, providing asynchronous HTTP servers, routing, middleware, extractors, and WebSocket support. |
| [Angular](https://github.com/BLCCoreStudio/OpenDevIndex/tree/framework/angular/entry) (`framework/angular`) | `software-development`, `web` | Full-featured web application framework maintained by Google with components, dependency injection, routing, forms, testing support, and an integrated development toolchain. |
| [Apache Flink](https://github.com/BLCCoreStudio/OpenDevIndex/tree/framework/apache-flink/entry) (`framework/apache-flink`) | `data`, `distributed-systems` | Distributed processing framework designed for stateful stream processing and batch workloads with event-time semantics and fault-tolerant execution. |
| [Apache Spark](https://github.com/BLCCoreStudio/OpenDevIndex/tree/framework/apache-spark/entry) (`framework/apache-spark`) | `analytics`, `data`, `distributed-systems` | Distributed data-processing engine for large-scale analytics with APIs for SQL, batch processing, streaming, machine learning, and graph workloads. |
| [ASP.NET Core](https://github.com/BLCCoreStudio/OpenDevIndex/tree/framework/aspnet-core/entry) (`framework/aspnet-core`) | `api`, `software-development`, `web` | Cross-platform open-source framework from Microsoft for building web applications, HTTP APIs, real-time services, and cloud backends on the modern .NET platform. |
...
| [Flutter](https://github.com/BLCCoreStudio/OpenDevIndex/tree/framework/flutter/entry) (`framework/flutter`) | `mobile`, `software-development` | Cross-platform application UI toolkit using Dart to build natively compiled mobile, web, desktop, and embedded interfaces from shared code. |
...
| [LlamaIndex](https://github.com/BLCCoreStudio/OpenDevIndex/tree/ai/llamaindex/entry) (`ai/llamaindex`) | `ai`, `data` | Framework for connecting language-model applications to private or external data through indexing, retrieval, workflows, and agent-oriented abstractions. |
...
| Module | Domains | Summary |
| --- | --- | --- |
| [axe-core](https://github.com/BLCCoreStudio/OpenDevIndex/tree/library/axe-core/entry) (`library/axe-core`) | `accessibility`, `testing`, `web` | Accessibility testing engine for automated detection of many WCAG-related issues in HTML-based user interfaces and browser testing workflows. |
| [DeepSpeed](https://github.com/BLCCoreStudio/OpenDevIndex/tree/library/deepspeed/entry) (`library/deepspeed`) | `ai`, `distributed-systems`, `machine-learning` | Deep-learning optimization library from Microsoft that provides memory, parallelism, communication, and training techniques for large neural-network workloads. |
| [JAX](https://github.com/BLCCoreStudio/OpenDevIndex/tree/ai/jax/entry) (`ai/jax`) | `ai`, `machine-learning` | Python array-computing and machine-learning library providing NumPy-style APIs with automatic differentiation, compilation, vectorization, and accelerator execution. |
| [Jest](https://github.com/BLCCoreStudio/OpenDevIndex/tree/library/jest/entry) (`library/jest`) | `testing`, `web` | JavaScript testing framework with assertions, mocking, snapshot testing, watch mode, and broad integration across web and Node.js projects. |
| [LightGBM](https://github.com/BLCCoreStudio/OpenDevIndex/tree/library/lightgbm/entry) (`library/lightgbm`) | `ai`, `data`, `machine-learning` | Gradient-boosting framework from Microsoft designed for efficient tree-based learning on large datasets, with support for classification, regression, and ranking. |
...
| [SQLAlchemy](https://github.com/BLCCoreStudio/OpenDevIndex/tree/library/sqlalchemy/entry) (`library/sqlalchemy`) | `data`, `python` | Python SQL toolkit and object-relational mapper that provides a composable SQL expression language, database connection management, and ORM patterns. |
| [Transformers](https://github.com/BLCCoreStudio/OpenDevIndex/tree/ai/transformers/entry) (`ai/transformers`) | `ai`, `machine-learning` | Hugging Face library providing pretrained transformer models, tokenizers, configuration, training utilities, and pipelines across major ML backends. |
| [XGBoost](https://github.com/BLCCoreStudio/OpenDevIndex/tree/library/xgboost/entry) (`library/xgboost`) | `ai`, `data`, `machine-learning` | Optimized gradient-boosting library for decision-tree models with distributed, GPU, and language-binding support across common machine-learning workflows. |

## runtime

| Module | Domains | Summary |
| --- | --- | --- |
| [Bun](https://github.com/BLCCoreStudio/OpenDevIndex/tree/runtime/bun/entry) (`runtime/bun`) | `developer-experience`, `programming-languages`, `web` | JavaScript and TypeScript runtime, package manager, bundler, and test runner focused on fast startup and an integrated developer-tooling experience. |
| [Deno](https://github.com/BLCCoreStudio/OpenDevIndex/tree/runtime/deno/entry) (`runtime/deno`) | `programming-languages`, `web` | JavaScript and TypeScript runtime with secure-by-default permissions, built-in developer tooling, web-standard APIs, and modern package compatibility. |
| [llama.cpp](https://github.com/BLCCoreStudio/OpenDevIndex/tree/ai/llama.cpp/entry) (`ai/llama.cpp`) | `ai`, `systems` | C and C++ inference project for running large language models efficiently across CPUs and multiple accelerator backends, including quantized local models. |
| [Node.js](https://github.com/BLCCoreStudio/OpenDevIndex/tree/runtime/nodejs/entry) (`runtime/nodejs`) | `programming-languages`, `web` | Cross-platform JavaScript runtime built on the V8 engine for server applications, command-line tools, networking services, and development automation. |
| [ONNX Runtime](https://github.com/BLCCoreStudio/OpenDevIndex/tree/library/onnxruntime/entry) (`library/onnxruntime`) | `ai`, `machine-learning`, `systems` | Cross-platform inference and training accelerator for models represented in ONNX, with execution providers targeting CPUs, GPUs, mobile devices, and specialized hardware. |
| [Tokio](https://github.com/BLCCoreStudio/OpenDevIndex/tree/library/tokio/entry) (`library/tokio`) | `networking`, `systems` | Asynchronous runtime and library ecosystem for Rust that provides task scheduling, networking primitives, timers, synchronization tools, and foundations for async applications. |
| [vLLM](https://github.com/BLCCoreStudio/OpenDevIndex/tree/ai/vllm/entry) (`ai/vllm`) | `ai`, `machine-learning` | High-throughput inference and serving engine for large language models, focused on efficient memory use, batching, and production model serving. |
| [Wasmtime](https://github.com/BLCCoreStudio/OpenDevIndex/tree/runtime/wasmtime/entry) (`runtime/wasmtime`) | `systems`, `webassembly` | Standalone WebAssembly runtime from the Bytecode Alliance focused on standards compliance, security, embedding, and efficient execution across platforms. |

## platform

| Module | Domains | Summary |
| --- | --- | --- |
| [Apache Kafka](https://github.com/BLCCoreStudio/OpenDevIndex/tree/platform/apache-kafka/entry) (`platform/apache-kafka`) | `data`, `distributed-systems` | Distributed event-streaming platform for durable publish-subscribe messaging, stream processing, and high-throughput data pipelines across services. |
...
| [MLflow](https://github.com/BLCCoreStudio/OpenDevIndex/tree/ai/mlflow/entry) (`ai/mlflow`) | `ai`, `devops`, `machine-learning` | Open-source platform for managing machine-learning experiments, model artifacts, evaluation, registries, and deployment-oriented lifecycle workflows. |
| [NATS](https://github.com/BLCCoreStudio/OpenDevIndex/tree/platform/nats/entry) (`platform/nats`) | `data`, `distributed-systems`, `networking` | High-performance messaging system for distributed applications, providing publish-subscribe, request-reply, queueing, persistence, and streaming through JetStream. |
| [PocketBase](https://github.com/BLCCoreStudio/OpenDevIndex/tree/platform/pocketbase/entry) (`platform/pocketbase`) | `api`, `data`, `developer-experience` | Compact open-source backend platform distributed as a single executable with an embedded database, realtime subscriptions, authentication, files, and an admin UI. |
| [Prometheus](https://github.com/BLCCoreStudio/OpenDevIndex/tree/cloud/prometheus/entry) (`cloud/prometheus`) | `observability` | Open-source monitoring and alerting system built around time-series metrics, label-based queries, pull-oriented scraping, and rule evaluation. |
| [RabbitMQ](https://github.com/BLCCoreStudio/OpenDevIndex/tree/platform/rabbitmq/entry) (`platform/rabbitmq`) | `data`, `distributed-systems` | Open-source message broker implementing AMQP and additional messaging protocols, widely used for work queues, routing, publish-subscribe, and reliable service integration. |
| [Redpanda](https://github.com/BLCCoreStudio/OpenDevIndex/tree/platform/redpanda/entry) (`platform/redpanda`) | `data`, `distributed-systems` | Kafka-compatible streaming data platform written primarily in C++ that provides a distributed log and event-streaming API without requiring a separate JVM runtime. |
| [Supabase](https://github.com/BLCCoreStudio/OpenDevIndex/tree/platform/supabase/entry) (`platform/supabase`) | `api`, `cloud`, `data` | Developer platform built around PostgreSQL that combines database, authentication, storage, realtime features, edge functions, and hosted or self-hosted workflows. |
| [ToolJet](https://github.com/BLCCoreStudio/OpenDevIndex/tree/platform/tooljet/entry) (`platform/tooljet`) | `developer-experience`, `web` | Open-source low-code development platform for internal tools, combining visual application building with database, API, and service integrations. |
| [Wazuh](https://github.com/BLCCoreStudio/OpenDevIndex/tree/security/wazuh/entry) (`security/wazuh`) | `observability`, `security` | Open-source security platform combining endpoint monitoring, log analysis, threat detection, compliance capabilities, and security analytics. |

## database

| Module | Domains | Summary |
| --- | --- | --- |
| [Apache Cassandra](https://github.com/BLCCoreStudio/OpenDevIndex/tree/database/cassandra/entry) (`database/cassandra`) | `data`, `distributed-systems` | Distributed wide-column database designed for high availability, horizontal scale, multi-node replication, and predictable write-heavy workloads without a single primary server. |
...
| [Milvus](https://github.com/BLCCoreStudio/OpenDevIndex/tree/database/milvus/entry) (`database/milvus`) | `ai`, `data` | Open-source vector database built for large-scale similarity search over embeddings, with indexing, filtering, distributed deployment, and AI retrieval workflows. |
| [MongoDB](https://github.com/BLCCoreStudio/OpenDevIndex/tree/database/mongodb/entry) (`database/mongodb`) | `data`, `distributed-systems`, `web` | Document-oriented database that stores records in BSON-like documents and provides indexing, replication, aggregation, transactions, and distributed deployment capabilities. |
| [MySQL](https://github.com/BLCCoreStudio/OpenDevIndex/tree/database/mysql/entry) (`database/mysql`) | `data` | Widely used relational database management system implementing SQL for transactional and web workloads with a large ecosystem of tools and integrations. |
| [Neo4j](https://github.com/BLCCoreStudio/OpenDevIndex/tree/database/neo4j/entry) (`database/neo4j`) | `analytics`, `data` | Graph database platform centered on property graphs and the Cypher query language, designed for traversing and analyzing highly connected data and relationship-heavy domains. |
| [OpenSearch](https://github.com/BLCCoreStudio/OpenDevIndex/tree/database/opensearch/entry) (`database/opensearch`) | `analytics`, `data`, `observability` | Open-source distributed search and analytics suite derived from Elasticsearch and Kibana, providing document indexing, querying, dashboards, and observability capabilities. |
...
| Module | Domains | Summary |
| --- | --- | --- |
| [GraphQL](https://github.com/BLCCoreStudio/OpenDevIndex/tree/protocol/graphql/entry) (`protocol/graphql`) | `api`, `web` | Query language and execution specification for APIs that lets clients request typed fields from a schema and supports queries, mutations, subscriptions, and introspection. |
...
| Module | Domains | Summary |
| --- | --- | --- |
| [Anthropic API](https://github.com/BLCCoreStudio/OpenDevIndex/tree/ai/anthropic-api/entry) (`ai/anthropic-api`) | `ai`, `api` | Developer API for integrating Anthropic Claude models into applications, including message-based model interaction and tool-oriented workflows. |
| [Gemini API](https://github.com/BLCCoreStudio/OpenDevIndex/tree/ai/gemini-api/entry) (`ai/gemini-api`) | `ai`, `api` | Google developer API for building applications with Gemini models through text, multimodal, structured, and tool-integrated model capabilities. |
| [OpenAI API](https://github.com/BLCCoreStudio/OpenDevIndex/tree/ai/openai-api/entry) (`ai/openai-api`) | `ai`, `api` | Developer API platform for integrating OpenAI models and capabilities into applications through authenticated programmable interfaces and official SDKs. |
